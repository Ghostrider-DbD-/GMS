...
_trigger setTriggerStatements ["{if (isPlayer _x) exitWith {1}} count thisList != 0;","_nul = [thisTrigger] call Zed_triggerActivated;","0 = [thisTrigger] spawn Zed_triggerDeactivated;"];

_trigger setVariable ["locationPos",(_this select 1)];
_trigger setVariable ["locationName",(_this select 2)];
_trigger setVariable ["spawnChance",(_this select 3)];
_trigger setVariable ["noSpawns",(_this select 4)];
