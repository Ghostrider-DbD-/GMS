...
if (toLower(GMSCore_modtype) isEqualTo "epoch") then {_item = "GroundWeaponHolder"};
_missionLootContainers = [];
for "_i" from 1 to (1 + floor(random(_level))) do
{
	private _lootPos = [ (_pos select 0) + (((random 5) + 5) * (selectRandom[1,-1])),(_pos select 1) + (((random 5) + 5) * (selectRandom[1,-1])), 0.05 ];
	//   ["Box_IND_AmmoOrd_F",[-25.7473,-46.3496,3.73],_crateLoot,_lootCounts,0],
	private _loot = missionNameSpace getVariable format["blck_boxLoot_%1",_difficulty];
	_missionLootContainers pushBack [_crateType,_lootPos,_loot,_lootCounts];
};
