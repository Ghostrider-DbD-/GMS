...
//[format["blck_fnc_spawnMissionAI: _this = %1",_this]] call blck_fnc_log;
...
{ 	
	{
		...
		private _newGroup = [_groupPos,_unitsToSpawn,_aiDifficultyLevel,patrolAreadDimensions,_uniforms,_headGear,_vests,_backpacks,_weapons,_sideArms,_isScubaGroup] call blck_fnc_spawnGroup;
		_groups pushBack _newGroup;		
		blck_monitoredMissionAIGroups pushback _newGroup;
		_allAI append (units _newGroup);
		//[format["blck_fnc_spawnMissionAI: _group %1 with %2 units added | _min %3 | _max %4 | _unitsToSpawn %5",_newGroup, count (units _newGroup),_min,_max,_unitsToSpawn]]call blck_fnc_log;				
	...
	if (_noAIGroups > 0) then
	{
		...
		private _groupPosns = [_coords,_noAIGroups,30,50] call blck_fnc_findPositionsAlongARadius;
		{
			_newGroup = [_x,_unitsPerGroup,_aiDifficultyLevel,patrolAreadDimensions,_uniforms,_headGear,_vests,_backpacks,_weapons,_sideArms,_isScubaGroup] call blck_fnc_spawnGroup;
			_groups pushBack _newGroup;
			blck_monitoredMissionAIGroups pushback _newGroup;
			_allAI append (units _newGroup);
			//[format["blck_fnc_spawnMissionAI: _group %1 with %2 units added",_newGroup, count (units _newGroup)]]	call blck_fnc_log;
		} forEach _groupPosns;
	};
};
//[format["blck_fnc_spawnMissionAI: _groups = %1 | _allAI = %2",_groups,_allAI]] call blck_fnc_log;
_allAI
