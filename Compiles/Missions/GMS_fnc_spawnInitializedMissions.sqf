...
for "_i" from 1 to (count blck_initializedMissionsList) do 
{
	...
	//  Check the timers and acitive mission counts. 
	private _missionInitialized = [_mission select 0,_mission select 1] call blck_fnc_initializeMission;

	// if _missionInitialized == false then it was not time for another mission of this type so add the mission back to the pending que
