		...
		if (random(1) < _chanceHeliPatrol) then
		{
			...
			if (typeName _temp isEqualTo "ARRAY") then
			{
				...
				if !(_abort) then
				{
					...
					diag_log format["[blckeagls] missionSpawner:: (293) heli AI spawned: _markerName %1 : _markerLabel %2 : count _blck_AllMissionAI = %3",_markerName,_markerLabel, count _blck_AllMissionAI];
				};
			};
		};
	};
...
if (blck_useVehiclePatrols &&  !(_submarinePatrolParameters isEqualTo []) ) then
{
	...
	_blck_AllMissionAI append (_temp select 1);
	diag_log format["[blckeagls] missionSpawner:: (374) submarine patrol AI spawned: _markerName %1 : _markerLabel %2 : count _blck_AllMissionAI = %3",_markerName,_markerLabel, count _blck_AllMissionAI];	
};
...
{
	diag_log format["[blckeagls] missionSpawner:: (333) spawning crate: _markerName %1 : _markerLabel %2 : _missionLootBoxes = %3",_markerName,_markerLabel, _missionLootBoxes];	
