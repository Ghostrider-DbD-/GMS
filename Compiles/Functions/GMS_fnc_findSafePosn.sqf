...
diag_log format["_fnc_findSafePosn: _isFlatCoords = %1 | _coords = %2",_isFlatCoords,_coords];

while { (_isFlatCoords isEqualTo [] || _coords isEqualTo []) && _count > 0} do 
{

	_coords = [blck_mapCenter,0,blck_mapRange,10,0,_slope,0,_blacklistedLocations] call BIS_fnc_findSafePos;
	//diag_log format["fnc_findSafePos: _coords = %1",_coords];
	...
	if (_coords isEqualTo [] || !(_isFlatCoords isEqualTo [])) then 
	{
		...
		_slope = _slope + 0.02;
		uiSleep 1;
		//diag_log format["_count = %1 | _slope = %2",_count,_slope];
	};

	//diag_log format["_fnc_findSafePosn: _isFlatCoords = %1 | _coords = %2 _count = %3",_isFlatCoords,_coords,_count];	
};
