...
*/
diag_log "[DBD Exile MS] Getting Locations for Zombie spawns";
		...
		if (_isAllowedPos) then {
			...
			if (_placeType in["NameVillage","NameLocal"]) then {
				if !(_placeName in ["military"]) then
				{
					...
					Zed_landLocations pushBack [_placeName,_placePos,_placeType];
...
{
	//diag_log format["[DEBUG :: setupLocations.sqf:: Zed_PatrolLocations includes _x =%1]",_x];
}forEach Zed_HoardLocations;

Zed_locationsInitialized = true;